...
설치하는 순간부터 최근 3개월 실제 기록을 보고 시작하니, 빈 화면에서 출발하지 않습니다.
...
**진짜 도움이 됐는지는 안 세도 됩니다.** 만든 시점의 습관 빈도와, 앞으로 그 명령어가 실제로 몇 번 쓰이는지를 AI_saver가 같이 기록해두고, 14일 뒤부터 리포트에 자동으로 이렇게 뜹니다.

```
## 승격한 명령어, 효과가 있었나

- `/focus-file` — 효과가 있습니다 (하루 발생률 82% 감소, 사용 6회). 계속 쓰세요.
- `/quick-test` — 30일 동안 1번밖에 안 쓰였습니다. 지워도 됩니다.
```

두 가지를 따로 봅니다 — **습관이 실제로 줄었는지**, 그리고 **명령어 자체를 진짜 썼는지**. 습관이 우연히 다른 이유로 줄었어도 명령어를 아무도 안 썼으면 지워도 됩니다. 리포트가 알려주는 폴더(`~/.claude/skills/<이름>/`)를 지우면 됩니다. 안 써보면 도움이 됐는지 알 수 없으니, 5번 이상 반복된 습관은 일단 만들어보고 이 판정에 맡기는 게 낫습니다.

## 6. 매번 안 물어봐도 자동으로 돌게 하기 (선택)

지금까지는 리뷰를 받으려면 "이번 달 리뷰 해줘"라고 매번 말해야 했습니다. Claude Code 앱의 예약 작업 기능을 쓰면 매달 1일 자동으로 돕니다.

> 매달 1일 오전 9시에 AI_saver 월간 리뷰(backfill → report → calibrate → wiki stats 확인 → 30일 20회 이상 습관 자동 승격 → 사용 안 하는 명령어 삭제 후보 표시)를 하는 예약 작업을 만들어줘. 실제 삭제는 하지 말고 후보만 남겨줘.

이렇게 말하면 됩니다. 이 예약 작업은 **이 컴퓨터에서, Claude Code 앱이 켜져 있을 때** 돕니다 (클라우드가 아닙니다 — 그래야 내 컴퓨터의 실제 기록에 접근할 수 있습니다). 앱이 꺼져 있으면 다음에 켤 때 대신 돕니다.
